...
const weatherStorageMocks = vi.hoisted(() => ({
  ...
  updateXiaomiLocationCache: vi.fn(),
}));

vi.mock("../../utils/weatherStorage", () => ({
  getValidXiaomiLocation: weatherStorageMocks.getValidXiaomiLocation,
  ...
  updateXiaomiLocationCache: weatherStorageMocks.updateXiaomiLocationCache,
}));
...
function installWeatherFetch(extra?: (url: string) => Promise<FetchResponseLike> | null) {
  const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
    ...
    if (url.includes("/wtr-v3/location/city/geo?"))
      return Promise.resolve(jsonResponse(xiaomiCity()));
    if (url.includes("/wtr-v3/location/city/search?"))
      return Promise.resolve(jsonResponse(xiaomiCity()));
    if (url.includes("/wtr-v3/weather/all?"))
      return Promise.resolve(jsonResponse(xiaomiWeatherAll()));
...
describe("weatherService - flow", () => {
  ...
  beforeEach(() => {
    ...
    weatherStorageMocks.getValidXiaomiLocation.mockReset();
    ...
    weatherStorageMocks.updateXiaomiLocationCache.mockReset();
    localStorage.clear();
  ...
  it("buildWeatherFlow：命中坐标与位置缓存，仅请求小米城市反查与天气详情", async () => {
    ...
    expect(
      fetchMock.mock.calls.filter(([input]) => String(input).includes("/wtr-v3/weather/all?"))
    ).toHaveLength(1);
  });

  it("全量与独立分钟接口复用 24 小时 locationKey 缓存", async () => {
    weatherStorageMocks.getValidCoords.mockReturnValue({
      lat: 31.2,
      lon: 121.5,
      source: "ip",
    ...
    weatherStorageMocks.getValidXiaomiLocation.mockReturnValue({
      lat: 31.2,
    ...
    const fetchMock = installWeatherFetch((url) => {
      if (url.includes("/wtr-v3/weather/xm/forecast/minutely?")) {
    ...
    const { buildWeatherFlow, fetchMinutelyPrecip } = await import("../weatherService");
    await buildWeatherFlow();
    await fetchMinutelyPrecip("121.5000,31.2000");

    const urls = fetchMock.mock.calls.map(([input]) => String(input));
    expect(urls.filter((url) => url.includes("/wtr-v3/location/city/geo?"))).toHaveLength(0);
    expect(urls.filter((url) => url.includes("/wtr-v3/weather/all?"))).toHaveLength(1);
    expect(
      urls.filter((url) => url.includes("/wtr-v3/weather/xm/forecast/minutely?"))
    ).toHaveLength(1);
    expect(
      urls
        .filter((url) => url.includes("/wtr-v3/weather/"))
        .every((url) => url.includes("locationKey=weathercn%3A101020100"))
    ).toBe(true);
  });
  ...
  it("buildWeatherFlow：手动城市定位会调用小米城市搜索并跳过浏览器定位", async () => {
    ...
    const fetchMock = installWeatherFetch((url) => {
      if (url.includes("restapi.amap.com/v3/geocode/regeo"))
        return Promise.resolve(
          jsonResponse({
            status: "1",
            regeocode: { formatted_address: "北京市 东城区", addressComponent: { city: "北京市" } },
          })
        );
      return null;
    });
    ...
    expect(
      fetchMock.mock.calls.some(([input]) =>
        String(input).includes("/wtr-v3/location/city/search?")
      )
    ).toBe(true);
  });
  ...
  it("buildLocationFlow：只刷新定位与反编码，不请求天气接口", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.includes("restapi.amap.com/v3/geocode/regeo"))
        return Promise.resolve(
          jsonResponse({
            status: "1",
            regeocode: { formatted_address: "上海市 黄浦区", addressComponent: { city: "上海市" } },
          })
        );
      return Promise.reject(new Error(`unexpected url: ${url}`));
    });
    ...
    expect(res.coords).toEqual({ lat: 31.2, lon: 121.5 });
    expect(
      fetchMock.mock.calls.some(([input]) => String(input).includes("/wtr-v3/weather/all?"))
    ).toBe(false);
  });
  ...
  it("buildWeatherFlow：反编码 Amap 失败时回退 OSM", async () => {
    ...
    installWeatherFetch((url) => {
      if (url.includes("restapi.amap.com/v3/geocode/regeo"))
        return Promise.resolve(jsonResponse({ status: "0", info: "INVALID_USER_KEY" }));
      if (url.includes("nominatim.openstreetmap.org/reverse"))
        return Promise.resolve(
          jsonResponse({ display_name: "Shanghai, China", address: { city: "Shanghai" } })
        );
      return null;
    });
  ...
  it("getCoordsViaAmapIP：解析 rectangle 并取中心点", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ status: "1", rectangle: "121,31;123,33" })
  ...
  it("getCoordsViaIP：当首个数据源失败时会回退到 ipinfo 的 loc", async () => {
    const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url === "https://ipinfo.io/json")
        return Promise.resolve(jsonResponse({ loc: "31.2,121.5" }));
      return Promise.reject(new Error(`unexpected url: ${url}`));
    });
    globalThis.fetch = fetchMock as unknown as typeof fetch;
  ...
  it("reverseGeocodeOSM：优先 neighbourhood，否则使用 suburb；无部件时回退 display_name", async () => {
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ address: { suburb: "S" }, display_name: "display-2" })
      ) as unknown as typeof fetch;
    ...
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ address: {}, display_name: "display-3" })
      ) as unknown as typeof fetch;
  ...
  it("reverseGeocodeAmap：status!=1 返回 error；无组件时回退 formatted_address", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(jsonResponse({ status: "0", info: "bad key" })) as unknown as typeof fetch;
  ...
  it("getCoordsViaAmapIP：rectangle 非法时返回 null", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValue(
        jsonResponse({ status: "1", rectangle: "not-a-rect" })
      ) as unknown as typeof fetch;
  ...
  it("getCoordsViaIP：可解析字符串纬度经度字段", async () => {
    const fetchMock = vi.fn().mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url === "https://ipapi.co/json/")
        return Promise.resolve(jsonResponse({ latitude: "31.2", longitude: "121.5" }));
      return Promise.reject(new Error(`unexpected url: ${url}`));
    });
    globalThis.fetch = fetchMock as unknown as typeof fetch;
