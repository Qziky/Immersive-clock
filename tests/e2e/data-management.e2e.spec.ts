...
test("清理临时缓存会保留设置和未注册的同源数据", async ({ page }) => {
  await page.addInitScript(() => {
    ...
    localStorage.setItem("weather-cache", JSON.stringify({ now: { temp: 20 } }));
  ...
  const stored = await page.evaluate(() => ({
    ...
    weather: localStorage.getItem("weather-cache"),
    backoff: localStorage.getItem("api-governance.hitokoto.block-until"),
  }));
  expect(stored.settings).toBe(settingsBefore);
  expect(stored.sentinel).toBe("keep");
  expect(stored.weather).toBeNull();
  expect(stored.backoff).toBeNull();
});
...
test("无效备份在预检阶段被拒绝且不会写入设置", async ({ page }) => {
  ...
  await dialog.locator('#data-settings-panel input[type="file"]').setInputFiles({
...
test("数据中心在桌面与窄屏下不显示原生文件框且没有横向溢出", async ({ page }) => {
  ...
  const nativeFileInput = panel.locator('input[type="file"]');
  ...
  for (const viewport of [
    ...
    expect(
      await nativeFileInput.evaluate((element) => {
