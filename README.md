...
- **环境感知**：
  ...
  - **噪音监测**：基于 AudioWorklet 的特征采样，以同一设备内可比较的相对环境评分为主；完成外部参考校准后可显示估算 dB(A)，并提供包含走势图的自习报告。
- **专注氛围**：
...
- **自习模式**：
  ...
  - **环境监测**：开启噪音监测后，评分低于提醒阈值会记录并提示；雨雪天气会有弹窗预警。
- **设置面板**：调整目标年份、噪音评分阈值、语录刷新间隔、自定义背景图等。

> 详细说明请见：
>
> - [使用说明](docs/usage.zh-CN.md)
> - [🎙️ 评分系统原理](docs/noise-scoring.md)
