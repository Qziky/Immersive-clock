...
在“设置 → 视觉外观”中，“基本”集中提供常用字体、背景以及时钟、倒计时、秒表、自习时间的样式
设置；语录、顶栏、天气等内容组件可从二级菜单直接进入。组件样式支持对子元素
设置颜色、透明度、字体、字重、字间距、阴影、表面、边框和圆角；组件与页面单独设置的
优先级高于基本设置。修改会实时预览，只有点击“保存”才会写入本地配置，取消会恢复已保存
外观。页面背景可继承基本背景，也可分别设置为预设、纯色或本地图片；多事件倒计时还可以
按事件单独覆盖样式。
