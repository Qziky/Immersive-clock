...
项目同时产出 Web/PWA、Electron 桌面包和 Docker/Nginx 镜像。构建入口由 `package.json`、
`vite.config.ts`、`electron-builder.json` 和 GitHub Actions 共同定义。
...
非 Electron 构建启用 `vite-plugin-pwa`：`registerType: "autoUpdate"`，自动生成 Service Worker
和 web manifest。预缓存静态资源，运行时缓存字体、图片、音频和 `/docs/*.md`。修改缓存规则、
资源路径或 manifest 时要验证：
...
EdgeOne 当前配置声明 Node 18，而仓库开发和 CI 要求 Node `>=20.19.0`；部署平台若执行构建应
...
4. 验证深链接、全屏、定位、仅音频权限、NTP IPC、公告和离线启动。
5. 检查版本注入、sitemap、robots、缓存 header 和 noindex header。
6. 在发布说明中记录构建版本、平台、测试命令和已知限制。
