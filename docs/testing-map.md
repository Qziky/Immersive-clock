...
- **设置/持久化**
  - AppSettings 深合并、局部更新、v3→v4 进度背景迁移、v4→v5 分钟降水弹窗字段清理、v5→v7 旧天气调度兼容、v7→v8 删除用户调度并迁移手动城市/经纬度、v10→v11 开发者模式默认关闭、时钟与自习秒数显示默认值/旧数据补齐、信息轮播约束与重置保留自定义消息：[appSettings.test.ts](file:///d:/Desktop/Immersive-clock/src/utils/__tests__/appSettings.test.ts) → [appSettings.ts](file:///d:/Desktop/Immersive-clock/src/utils/appSettings.ts) / [storageInitializer.ts](file:///d:/Desktop/Immersive-clock/src/utils/storageInitializer.ts)
  ...
  - 外观“整体样式/时间显示/顶部信息栏”导航、时间与信息栏分段选择、时钟/自习秒数独立草稿与预览及其对应“主时间/秒钟”调整对象归属、默认今日进度预览及其无障碍名称、正式展示层复用、按当前组件区域自适应取景、栏体四边等距框选、事件倒计时独立框选层与防裁剪、预览描边仅在点击对象或状态标签后切换、噪音状态样例、语录 `QuoteReveal` 结构、秒表预览静态可见性、对象与属性恢复、状态样式、倒计时指定事件、背景来源切换与刷新持久化：`src/components/SettingsPanel/__tests__/AppearanceSettingsPanel.test.tsx` + `src/components/Clock/__tests__/Clock.test.tsx` + `src/components/Study/__tests__/StudyTimePresentation.test.tsx` + `src/ui/components/__tests__/Tabs.test.tsx` + `tests/e2e/settings-motion.e2e.spec.ts` + `tests/e2e/settings-persistence.e2e.spec.ts` + `tests/e2e/visual-regression.e2e.spec.ts`
...
- 同时解析今日 24 小时与课时/课间进度快照、分时问候、午夜重置、课时空态、信息与背景原子切换及设置保存事件先于持久化时的下一任务重读：`src/components/StudyStatus/__tests__/StudyStatus.test.tsx` → `src/components/StudyStatus/StudyStatus.tsx` / `src/components/StudyStatus/dayGreeting.ts`
  ...
  - 噪音监测四标签的顺序、实时与统计合并展示、内容隔离、可访问关联与跨标签草稿保持：`src/components/SettingsPanel/sections/__tests__/StudySettingsPanel.test.tsx` → `src/components/SettingsPanel/sections/StudySettingsPanel.tsx`
  - 项目信息中的版本、授权及服务与隐私说明，以及错误与调试中的开发者模式草稿、保存、组件规范与音频诊断入口可见性：[AboutSettingsPanel.test.tsx](file:///d:/Desktop/Immersive-clock/src/components/SettingsPanel/__tests__/AboutSettingsPanel.test.tsx) → [AboutSettingsPanel.tsx](file:///d:/Desktop/Immersive-clock/src/components/SettingsPanel/sections/AboutSettingsPanel.tsx)
  - 组件规范和音频诊断路由仅允许开发者模式访问，开发者路径识别及 `robots`/`googlebot` 动态禁索引策略：[App.test.tsx](file:///d:/Desktop/Immersive-clock/src/App.test.tsx) + [developerPages.test.ts](file:///d:/Desktop/Immersive-clock/src/utils/__tests__/developerPages.test.ts) → [App.tsx](file:///d:/Desktop/Immersive-clock/src/App.tsx) / [developerPages.ts](file:///d:/Desktop/Immersive-clock/src/utils/developerPages.ts)
...
- **天气**
  - 高精度浏览器参数、无低精度重试、公共 IP 降级、自动模式拒绝手动缓存、城市多候选与 24 小时查询/坐标解析缓存、手动 `locationKey` 直用及旧坐标迁移：`src/services/__tests__/weatherService.flow.test.ts` → `src/services/locationService.ts` / `src/services/weatherService.ts`
  - 单次 `/weather/all`、原子快照、内嵌分钟优先与独立分钟降级、并发单飞/强制排队、公共 IP 后 30 分钟浏览器重试、离线、过期、退避、限流和跨标签页重读：`src/services/__tests__/weatherRuntime.test.ts` + `src/services/__tests__/weatherSyncChannel.test.ts` → `src/services/weatherRuntime.ts` / `src/services/weatherSyncChannel.ts`
  - 当前/指数/AQI/污染物/逐时逐日/昨日/前一小时/预警防御与图片/台风/技术元数据的完整归一化和原始响应保留：`src/services/__tests__/weatherService.test.ts` → `src/services/weatherService.ts`
  - 固定内部请求保护、全量/分钟 60 秒硬间隔、城市请求 1 秒间隔、滚动小时上限、429 `Retry-After`、403 两小时冷却，以及 Web Locks/localStorage 按请求键租约：`src/services/__tests__/weatherRequestGuard.test.ts` + `src/services/__tests__/weatherCrossTabLock.test.ts` + `src/services/__tests__/xiaomiWeatherClient.test.ts` + `src/services/__tests__/xiaomiWeatherProxy.test.ts` → `src/services/weatherRequestGuard.ts` / `src/services/weatherCrossTabLock.ts` / `src/services/xiaomiWeatherClient.ts` / `electron/xiaomiWeatherProxy.ts`
  ...
  - 共享分钟快照只做倒计时和阶段重算；设置页的系统更新状态、手动城市强制选择、七类天气数据与失败/过期渲染：`src/services/__tests__/minutelyWeatherRuntime.test.ts` + `src/components/SettingsPanel/sections/__tests__/WeatherSettingsPanel.test.tsx` + `src/components/Weather/__tests__/Weather.test.tsx` + `tests/e2e/settings-persistence.e2e.spec.ts` → `src/services/minutelyWeatherRuntime.ts` / `src/components/SettingsPanel/sections/WeatherSettingsPanel.tsx` / `src/components/SettingsPanel/sections/WeatherLivePanel.tsx`
  - 天气网络边界确保只有统一运行时可调用全量和分钟入口，生产代码不再使用旧协调器、刷新层或 DOM 天气事件：`src/services/__tests__/weatherRuntime.boundary.test.ts` → `src/services/weatherRuntime.ts` / `src/services/weatherService.ts` / `src/services/locationService.ts`
  ...
  - 预警快照接收全量注入并排序、去重、过期清理；顶部天气隐藏后预警、AQI、日出日落提醒仍独立运行且保持去重：`src/services/__tests__/weatherAlertRuntime.test.ts` + `src/services/__tests__/weatherNotificationRuntime.test.ts` + `src/components/StudyStatus/__tests__/StudyStatus.test.tsx` + `tests/e2e/study-smoke.e2e.spec.ts` → `src/services/weatherAlertRuntime.ts` / `src/services/weatherNotificationRuntime.ts` / `src/components/StudyStatus/StudyStatus.tsx`
- **时间同步**
...
- **噪音**
  - AudioWorklet 特征提取覆盖 44.1/48 kHz 帧长、序列、样本位置、A 加权、RMS、P1 谷值、异常零信号、削波和 PCM 不外泄；健康监测将处理异常与连续零值统一为“信号异常”：`src/services/noise/__tests__/noiseFeatureExtractor.test.ts` + `src/services/noise/__tests__/noiseSignalHealthMonitor.test.ts` → `src/services/noise/noiseAudioWorklet.ts` / `src/services/noise/noiseFeatureExtractor.ts` / `src/services/noise/noiseSignalHealthMonitor.ts`
  - `spectral-activity-v2` 覆盖低灵敏度帧活动映射、秒中位数、P20、60 秒覆盖率、异常零信号排除、事件迟滞/合并和 5 秒滚动窗口：`src/utils/__tests__/noiseScoreEngine.test.ts` + `src/services/noise/__tests__/noiseCaptureRuntime.test.ts` → `src/utils/noiseScoreEngine.ts` / `src/services/noise/noiseCaptureRuntime.ts`
  - 麦克风枚举去重、显式授权释放临时轨道、指定设备约束、缺失回退、权限拒绝不回退与设备变化订阅：`src/services/noise/__tests__/noiseInputDeviceService.test.ts` → `src/services/noise/noiseInputDeviceService.ts`
  - 采集 Runtime 覆盖会话协议、60 秒进度、刷新后从同设备最近检查点续接实时评分、跨会话续接不污染新历史切片来源、5 秒检查点、持久化失败不中断评分及独立 dB(A) 校准：`src/services/noise/__tests__/noiseCaptureRuntime.test.ts` + `src/services/noise/__tests__/noiseDeviceProfileService.test.ts` → `src/services/noise/noiseCaptureRuntime.ts` / `src/services/noise/noiseDeviceProfileService.ts`
  - Web Locks/localStorage 租约的单 Leader、Follower 零采集写入、epoch/乱序过滤、失效接管和历史写锁：`src/services/noise/__tests__/noiseCoordinator.test.ts` + `src/services/noise/__tests__/noiseHistoryLock.test.ts` + `src/services/noise/__tests__/noiseStreamService.test.ts` + `tests/e2e/noise-multitab.e2e.spec.ts` → `src/services/noise/noiseCoordinator.ts` / `src/services/noise/noiseHistoryLock.ts` / `src/services/noise/noiseStreamService.ts`
  - IndexedDB v7 新库与 v6→v7 升级删除旧 Store/旧模型评分、TypedArray 分块、原子检查点、崩溃恢复、同设备相邻会话的最近 60 秒评分恢复、启动期旧键清理与后台重算、14 天清理和重叠会话单来源选择：`src/utils/__tests__/db.test.ts` + `src/services/noise/__tests__/noiseDataMaintenance.test.ts` + `src/services/noise/__tests__/noiseFeatureRepository.test.ts` + `src/utils/__tests__/noiseSliceService.test.ts` → `src/utils/db.ts` / `src/services/noise/noiseDataMaintenance.ts` / `src/services/noise/noiseFeatureRepository.ts` / `src/utils/noiseSliceService.ts`
  - `.icnoise` 二进制往返、预检、幂等新增占用和损坏文件零写入：`src/services/noise/__tests__/noiseFeatureArchiveService.test.ts` → `src/services/noise/noiseFeatureArchiveService.ts`
  - 派生评分删除后重建、暂停续跑、配置摘要失配、幂等写入和不区分原始来源状态的全部旧模型缓存清理：`src/services/noise/__tests__/noiseRescoreService.test.ts` → `src/services/noise/noiseRescoreService.ts` / `src/services/noise/noiseRescore.worker.ts`
  - AppSettings v8→v9→v10→v11、旧显示开关初始化采集开关、麦克风偏好规范化、噪音报告自动关闭时长默认值与边界、噪音设置无变化保存短路、真实变化广播，以及备份噪音域 v4 往返、v1-v3 明确拒绝和无噪音域设置备份兼容：`src/utils/__tests__/appSettings.test.ts` + `src/utils/__tests__/noiseControlSettings.test.ts` + `src/utils/__tests__/noiseReportSettings.test.ts` + `src/services/__tests__/dataManagement.test.ts` → `src/utils/appSettings.ts` / `src/utils/noiseControlSettings.ts` / `src/utils/noiseReportSettings.ts` / `src/services/dataManagement.ts`
  - 实时监控通过公共折线图渲染评分或已校准估算值，未校准不显示 dB(A) 数字；监控器与调试页统一展示“信号异常”而不猜测异常来源：`src/components/NoiseSettings/__tests__/RealTimeNoiseChart.test.tsx` + `src/components/NoiseMonitor/__tests__/NoiseMonitor.test.tsx` + `src/pages/Debug/__tests__/AudioDebugPage.test.tsx` → `src/components/NoiseSettings/RealTimeNoiseChart.tsx` / `src/components/NoiseMonitor/NoiseMonitor.tsx` / `src/pages/Debug/AudioDebugPage.tsx`
  - URL 专用音频调试页受开发者模式路由守卫保护，并按输入设备、采集会话、特征提取、信号诊断、评分与校准、分发与持久化展示完整流程，覆盖动态摘要、默认 40 dB(A) 的外部参考校准/清除、跨标签/IndexedDB 分支、设备选择、三条正式特征曲线、统一“信号异常”、按需临时会话及 1440×900、390×844、320×568 轨道和横向溢出：`src/App.test.tsx` + `src/pages/Debug/__tests__/AudioDebugPage.test.tsx` + `tests/e2e/audio-debug.e2e.spec.ts` → `src/App.tsx` / `src/pages/Debug/AudioDebugPage.tsx` / `src/services/noise/noiseInputDeviceService.ts` / `src/services/noise/noiseStreamService.ts`
  - 数据设置页覆盖原始帧占用、持久存储、`.icnoise` 预检导入、后台重算状态，以及清理采集会话/100 ms 原始帧/派生评分/重算状态并保留监测设置和 dB(A) 校准的提示：`src/components/SettingsPanel/__tests__/DataSettingsPanel.test.tsx` → `src/components/SettingsPanel/sections/DataSettingsPanel.tsx`
  - 噪音历史与报告共享滚动窗口新增区间聚合，覆盖重复窗口、边界裁剪、低覆盖、时间缺口和校准权重，确保有效覆盖率不超过 100%：`src/utils/__tests__/noiseReportAggregation.test.ts` + `src/utils/__tests__/noiseHistoryBuilder.test.ts` → `src/utils/noiseReportAggregation.ts` / `src/utils/noiseHistoryBuilder.ts`
  - 噪音报告采用结论优先布局，以平均分和安静达标率为摘要，并按提醒线显示达标/需留意状态；评分区域展示三项实际约扣分及其占比，并生成总体表现、主要影响、最低记录和数据范围解读，不向用户展开采样实现规则；课时自动弹出的报告按 1–60 分钟设置自动关闭，历史详情保持打开，同时覆盖空态、零扣分、三类主要影响、覆盖有限提示及 1440×900、390×844、320×568 的弹窗边界和横向溢出：`src/components/NoiseReportModal/__tests__/NoiseReportModal.test.tsx` + `src/components/Study/__tests__/Study.test.tsx` + `tests/e2e/modal-redesign.e2e.spec.ts` + `tests/e2e/settings-persistence.e2e.spec.ts` → `src/components/NoiseReportModal/NoiseReportModal.tsx` / `src/components/Study/Study.tsx`
- **公告**
  ...
  - 公告弹窗无操作 120 秒自动关闭、最后 60 秒提示、点击/触控/键盘/滚动重置（指针移动不续时）、反馈问卷暂停及隐藏偏好复用：`src/components/AnnouncementModal/__tests__/AnnouncementModal.test.tsx` → `src/components/AnnouncementModal/AnnouncementModal.tsx`
- **通用**
  ...
  - 公共折线图的无障碍名称与描述、阈值、面积、断点、多序列、附加柱层、图例、无额外辉光描边和空态：`src/ui/components/__tests__/Chart.test.tsx` → `src/ui/components/Chart.tsx`
  ...
  - Electron 全屏权限只允许应用主窗口的主框架，拒绝子框架和其他窗口：`src/utils/__tests__/electronPermissionPolicy.test.ts` → `electron/permissionPolicy.ts` / `electron/main.ts`
  ...
  - SettingsShell 分组展开、展开动画结束后切换目标项、每组最近条目、紧凑 rail/子菜单、遮罩、Escape 焦点恢复、drawer variant、无内容页头时正文与底栏结构，以及减少动态效果下即时切换：`src/ui/components/__tests__/SettingsShell.test.tsx` → `src/ui/components/SettingsShell.tsx`
...
- 设置分组导航、麦克风选择与噪音报告自动关闭时长的草稿/保存/取消/重载、天气/定位内容隔离、定位设置与状态同页展示、系统自适应更新状态、手动城市搜索选择与无选择禁止保存、七标签实时详情，以及 1440×900/390×844/320×568 无溢出和控制台检查：[settings-persistence.e2e.spec.ts](file:///d:/Desktop/Immersive-clock/tests/e2e/settings-persistence.e2e.spec.ts)
- 同一浏览器上下文双标签页并发启动时只发出一次 `/weather/all`，复用 v2 位置快照，不额外请求城市解析或独立分钟接口，并通过缓存同步消息重读共享快照：`tests/e2e/weather-coordination.e2e.spec.ts`
- 同一浏览器上下文双标签页只调用一次 `getUserMedia`、共享相同摘要、只写一份切片、跨标签不传 PCM，Leader 关闭后 5 秒内由 Follower 接管：`tests/e2e/noise-multitab.e2e.spec.ts`
- 开发者模式关闭时组件规范和音频调试直达均重定向首页；开启后两页可访问且 `robots`/`googlebot` 保持 `noindex, nofollow, noarchive`：`tests/e2e/developer-pages.e2e.spec.ts`
- 音频调试页仅在开发者模式开启后通过 `/debug/audio` 访问，麦克风按用户操作启动且可在采集中切换设备，支持 10 秒外部参考 dB(A) 校准，并按紧凑六阶段流程和大尺寸曲线图展示确定性特征、处理配置、跨标签快照与 IndexedDB 分支；临时会话不写历史，覆盖 1440×900 的流程高度/明细行密度及 390×844、320×568 的图表高度、纵向轨道和无横向溢出：`tests/e2e/audio-debug.e2e.spec.ts`
...
- 开启开发者模式后的 DesignSystem Catalog 各 section、SettingsShell 无内容页头底栏状态、浮层固定打开态及 `1440×900`、`390×844`、`320×568` 的 Windows Edge 串行视觉基线：`tests/e2e/design-system-visual.e2e.spec.ts`
