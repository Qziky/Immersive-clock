...
- **设置/持久化**
  - AppSettings 深合并、局部更新、v3→v4 进度背景迁移、v4→v5 分钟降水弹窗字段清理、v5→v7 旧天气调度兼容、v7→v8 删除用户调度并迁移手动城市/经纬度、信息轮播约束与重置保留自定义消息：[appSettings.test.ts](file:///d:/Desktop/Immersive-clock/src/utils/__tests__/appSettings.test.ts) → [appSettings.ts](file:///d:/Desktop/Immersive-clock/src/utils/appSettings.ts) / [storageInitializer.ts](file:///d:/Desktop/Immersive-clock/src/utils/storageInitializer.ts)
...
- **噪音**
  ...
  - 采集 Runtime 覆盖会话协议、60 秒进度、5 秒检查点、持久化失败不中断评分及独立 dB(A) 校准：`src/services/noise/__tests__/noiseCaptureRuntime.test.ts` + `src/services/noise/__tests__/noiseDeviceProfileService.test.ts` → `src/services/noise/noiseCaptureRuntime.ts` / `src/services/noise/noiseDeviceProfileService.ts`
  ...
  - IndexedDB v7 新库与 v6→v7 升级删除旧 Store/旧模型评分、TypedArray 分块、原子检查点、崩溃恢复、启动期旧键清理与后台重算、14 天清理和重叠会话单来源选择：`src/utils/__tests__/db.test.ts` + `src/services/noise/__tests__/noiseDataMaintenance.test.ts` + `src/services/noise/__tests__/noiseFeatureRepository.test.ts` + `src/utils/__tests__/noiseSliceService.test.ts` → `src/utils/db.ts` / `src/services/noise/noiseDataMaintenance.ts` / `src/services/noise/noiseFeatureRepository.ts` / `src/utils/noiseSliceService.ts`
  ...
  - AppSettings v8→v9→v10、旧显示开关初始化采集开关、麦克风偏好规范化、噪音设置无变化保存短路、真实变化广播，以及备份噪音域 v4 往返、v1-v3 明确拒绝和无噪音域设置备份兼容：`src/utils/__tests__/appSettings.test.ts` + `src/utils/__tests__/noiseControlSettings.test.ts` + `src/services/__tests__/dataManagement.test.ts` → `src/utils/appSettings.ts` / `src/utils/noiseControlSettings.ts` / `src/services/dataManagement.ts`
  ...
  - URL 专用音频调试页按输入设备、采集会话、特征提取、信号诊断、评分与校准、分发与持久化展示完整流程，覆盖动态摘要、外部参考 dB(A) 校准/清除、跨标签/IndexedDB 分支、设备选择、三条正式特征曲线、统一“信号异常”、按需临时会话及 1440×900、390×844、320×568 轨道和横向溢出：`src/pages/Debug/__tests__/AudioDebugPage.test.tsx` + `tests/e2e/audio-debug.e2e.spec.ts` → `src/pages/Debug/AudioDebugPage.tsx` / `src/services/noise/noiseInputDeviceService.ts` / `src/services/noise/noiseStreamService.ts`
  ...
  - 噪音报告采用结论优先布局，以总评分和安静达标率为摘要，将覆盖率降级为卡片外的数据质量细注；以评分为主图，只在切片自带校准快照时提供估算 dB(A) 模式，并覆盖空态、覆盖有限提示及 1440×900、390×844、320×568 的弹窗边界和横向溢出：`src/components/NoiseReportModal/__tests__/NoiseReportModal.test.tsx` + `tests/e2e/modal-redesign.e2e.spec.ts` → `src/components/NoiseReportModal/NoiseReportModal.tsx`
- **公告**
...
- **通用**
  ...
  - SettingsShell 分组展开、每组最近条目、紧凑 rail/子菜单、遮罩、Escape 焦点恢复、drawer variant、无内容页头时正文与底栏结构和减少动态效果：`src/ui/components/__tests__/SettingsShell.test.tsx` → `src/ui/components/SettingsShell.tsx`
...
- 设置分组导航、麦克风选择草稿与持久化、天气/定位内容隔离、定位设置与状态同页展示、系统自适应更新状态、手动城市搜索选择与无选择禁止保存、七标签实时详情，以及 1440×900/390×844/320×568 无溢出和控制台检查：[settings-persistence.e2e.spec.ts](file:///d:/Desktop/Immersive-clock/tests/e2e/settings-persistence.e2e.spec.ts)
...
- 音频调试页仅通过 `/debug/audio` 直达，麦克风按用户操作启动且可在采集中切换设备，支持 10 秒外部参考 dB(A) 校准，并按紧凑六阶段流程和大尺寸曲线图展示确定性特征、处理配置、跨标签快照与 IndexedDB 分支；临时会话不写历史，覆盖 1440×900 的流程高度/明细行密度及 390×844、320×568 的图表高度、纵向轨道和无横向溢出：`tests/e2e/audio-debug.e2e.spec.ts`
...
- DesignSystem Catalog 各 section、SettingsShell 无内容页头底栏状态、浮层固定打开态及 `1440×900`、`390×844`、`320×568` 的 Windows Edge 串行视觉基线：`tests/e2e/design-system-visual.e2e.spec.ts`
