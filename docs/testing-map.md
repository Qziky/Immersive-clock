...
- **噪音**
  ...
  - AppSettings v8→v9→v10、旧显示开关初始化采集开关、麦克风偏好规范化、备份噪音域 v4 往返、v1-v3 明确拒绝及无噪音域设置备份兼容：`src/utils/__tests__/appSettings.test.ts` + `src/services/__tests__/dataManagement.test.ts` → `src/utils/appSettings.ts` / `src/services/dataManagement.ts`
  ...
  - 噪音历史构建仅统计有效切片，并区分有效覆盖与排除时长：`src/utils/__tests__/noiseHistoryBuilder.test.ts` → `src/utils/noiseHistoryBuilder.ts`
  - 噪音报告以评分为主图，只在切片自带校准快照时提供估算 dB(A) 模式：`src/components/NoiseReportModal/__tests__/NoiseReportModal.test.tsx` → `src/components/NoiseReportModal/NoiseReportModal.tsx`
- **公告**
