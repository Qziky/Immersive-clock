...
Web/PWA 会在启动时后台检查稳定版；回到前台且距上次检查超过 6 小时时再次检查。发现新版本后，
右下角会持续显示非阻塞提醒，不会自动刷新正在使用的页面。点击“立即更新”后才切换到新 Service
Worker 并刷新当前页面。若新清单已经发布但资源尚未就绪，会显示“资源准备中”，可稍后重试。

也可以打开“设置 → 系统数据 → 应用更新”查看当前/最新版本、上次检查时间和状态，或手动点击
“检查更新”。“自动检查更新”默认开启，可在设置草稿中关闭后保存；手动检查始终可用。
...
关闭更新提醒只对当前这次运行有效，下次启动仍会提示。点击主界面右下角版本号仍用于查看公告和
更新日志，不替代“应用更新”面板。

## 更新桌面客户端

Windows NSIS 安装版和 Linux AppImage 会检查稳定版并在后台静默下载。下载完成后可点击“重启并
安装”；如果暂不操作，正常退出应用时会自动安装已下载版本。Windows 便携版与 Linux deb/rpm
不会自替换，更新动作会打开对应下载地址或 GitHub Release 页面。

若自动下载失败，可在“设置 → 系统数据 → 应用更新”重试或打开发布页。生产自动更新是否能被
系统无警告接受，仍取决于该版本的代码签名状态。
