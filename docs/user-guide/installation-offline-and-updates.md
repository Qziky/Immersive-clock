...
可以打开“设置 → 系统数据 → 应用更新”查看当前/最新版本、上次检查时间和状态。点击“检查更新”
可立即查询当前平台的稳定版，即使“自动检查更新”已关闭也可使用。“自动检查更新”默认开启，
可在设置草稿中关闭后保存；该页面不提供手动安装操作。浏览器已经发现的新 Service Worker
仍会直接应用，以避免继续运行失效的旧资源。
...
若自动下载失败，可在“设置 → 系统数据 → 应用更新”查看错误；恢复网络后可点击“检查更新”
立即重试，也可以重启应用以重新触发自动检查。
